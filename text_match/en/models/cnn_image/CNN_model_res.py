...
class cnn_text:
    ...
    def inference(self):
        ...
        with tf.variable_scope("conv1"):
            self.res = tf.layers.conv2d(inputs=self.image_exp, filters=128, kernel_size=1, strides=[1, 1],
                                        padding='SAME', use_bias=True,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(), name="feature_map")
            self.input1 = tf.nn.relu(self.res)
            ...
            self.conv2 = tf.layers.conv2d(inputs=self.activ1, filters=128, kernel_size=3, strides=[1, 1],
                                          padding="SAME",
                                          use_bias=True,
            ...
            self.bn2 = tf.layers.batch_normalization(inputs=self.conv2, axis=3, momentum=_BATCH_NORM_DECAY,
                                                     epsilon=_BATCH_NORM_EPSILON, center=True,
                                                     scale=True, training=self.is_training, fused=True)
            self.activ2 = self.bn2 + self.input1
            self.activ2 = tf.nn.relu(self.activ2)
