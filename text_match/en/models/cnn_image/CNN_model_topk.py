...
class cnn_text:
    ...
    def inference(self):
        ...
        with tf.variable_scope("conv3"):
            ...
            self.trop = tf.transpose(self.activ3, [0, 1, 3, 2])
            self.topk = tf.nn.top_k(self.trop, k=8)
            self.inputs = tf.layers.flatten(self.topk[0])
            # self.pool3 = tf.keras.layers.GlobalAveragePooling2D()(self.activ3)
        # self.fc = tf.layers.flatten(self.pool2, name="flatten")
        with tf.name_scope("outputs"):
            self.fc1 = tf.layers.dense(self.inputs, 512, activation=tf.nn.relu, use_bias=True,
