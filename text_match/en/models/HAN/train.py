...
from text_match.en.models.HAN.HAN_model_dist_bn import HierarchicalAttention
import datetime
import os
import time
...
def train(x_train1, x_dev1, x_train2, x_dev2, y_train, y_dev, word_embedding, max_len, vocab_size):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            rnn = HierarchicalAttention(FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps, FLAGS.decay_rate,
                                        max_len,
            ...
            checkpoint_dir = os.path.abspath(
                os.path.join(os.path.curdir, "checkpoint", timestamp + "_bn"))
            ...
            for batch in batches:
                ...
                # if current_step % FLAGS.evaluate_every == 0:
                #     print("\nEvaluation:")
                #     validation_loss = dev_step(x_dev1, x_dev2, y_dev)
                #     if validation_loss < best_validation_loss:
                #         best_validation_loss = validation_loss
                #         path = saver.save(sess, checkpoint_prefix, global_step=current_step)
                #         print("Saved model checkpoint to {}\n".format(path))
                #         i = current_step
                #     elif (current_step - i) > EARLY_STOP_PATIENCE:
                #         print("best loss: {:g}, steps: {}".format(best_validation_loss, i))
                #         log_write.write("best loss: {:g}, steps: {}".format(best_validation_loss, i))
                #         print("Early Stoping")
                #         log_write.flush()
                #         break
            log_write.close()
