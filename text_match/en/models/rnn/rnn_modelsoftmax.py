...
class rnn_dot:
    ...
    def inference(self):
        ...
        with tf.variable_scope("outputs"):
            ...
            #
            self.fc1 = tf.layers.dense(self.input_dense, 256, activation=tf.nn.leaky_relu)
            ...
            self.fc = tf.layers.dense(self.fc3, 2, activation=tf.nn.relu)

            self.softmax = tf.nn.softmax(self.fc)
            self.logits = self.softmax[:, 1]
