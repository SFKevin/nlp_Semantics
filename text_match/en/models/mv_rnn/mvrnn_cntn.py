...
class MV_RNN:
    ...
    def inference(self):
        ...
        self.inputs = tf.einsum('abcd,de->abce', self.output, self.U1)
        self.inputs = tf.squeeze(self.inputs, axis=-1)
        self.inputs = tf.nn.relu(self.inputs)
        self.kmax = tf.nn.top_k(self.inputs, k=8, name="k-max-pool")
