...
class MV_RNN:
    ...
    def inference(self):
        ...
        self.W1 = tf.get_variable(name="W1", shape=[3, 2 * self.hidden_size, 2 * self.hidden_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
        self.V1 = tf.get_variable(name="V1", shape=[4 * self.hidden_size, self.sequence_length, 3], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
        # self.b = tf.get_variable(name="bias", shape=[3], dtype=tf.float32,
        #                          initializer=tf.contrib.layers.xavier_initializer())
        self.U1 = tf.get_variable(name="U", shape=[3, 1], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
        self.rnn1_rnn2 = tf.concat([self.outputs1_rnn, self.outputs2_rnn], axis=2)
        self.f0 = tf.einsum('abc,cde->abde', self.rnn1_rnn2, self.V1)
        f1_list = []
        for i in range(3):
            temp0 = tf.einsum('abc,cd->abd', self.outputs1_rnn, self.W1[i])
            temp1 = tf.matmul(temp0, tf.transpose(self.outputs2_rnn, [0, 2, 1]))
            f1_list.append(temp1)
        self.output = tf.stack(f1_list, axis=3)
        self.output = tf.add(self.output, self.f0)

        with tf.variable_scope("conv1"):
            self.conv1 = tf.layers.conv2d(self.output, filters=128, kernel_size=3, strides=[1, 1], padding='SAME',
                                          use_bias=True,
        ...
        with tf.variable_scope("conv2"):
            self.conv2 = tf.layers.conv2d(self.pool1, filters=128, kernel_size=3, strides=[1, 1], padding='SAME',
                                          use_bias=True,
        ...
        with tf.variable_scope("conv3"):
            self.conv3 = tf.layers.conv2d(inputs=self.pool2, filters=512, kernel_size=3, strides=[1, 1], padding="SAME",
                                          use_bias=True,
