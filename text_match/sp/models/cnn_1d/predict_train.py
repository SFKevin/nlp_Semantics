...
if __name__ == '__main__':
    x_train1, x_train2, y_train, _, _, _, word_embedding, max_len, vocab_size, x_test1, x_test2 = process()
    # result = predict(x_train1, x_train2)
    # timestamp = str(int(time.time()))
    # file_name = timestamp + "train_cnn1d.txt"
    file_name1 = "1532881338y_train_cnn1d.txt"
    # np.savetxt(file_name, result)
    np.savetxt(file_name1, y_train, fmt="%d")
    # print("number of result: %d" % len(result))
    print("number of y: %d" % len(y_train))
