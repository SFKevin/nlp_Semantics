...
def process():
    ...
    return x_train1, x_dev1, x_train2, x_dev2, y_train, y_dev, word_embedding, max_len, vocab_size, x_test1, x_test2
...
def predict(x_pre1, x_pre2):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            checkpoint_dir = os.path.abspath(os.path.join(os.path.curdir, "checkpoint", "1532706241"))
            model_path = os.path.join(checkpoint_dir, "model-5100.meta")
            ...
            results = []
            # i = 0
            test_batch = datahelper.batch_iter(list(zip(x_test1, x_test2)), FLAGS.batch_size, 1, shuffle=False)
            for test_data in test_batch:
                ...
                logits = sess.run(result,
                                  {"is_training:0": False, "input_x1:0": x_dev_batch1, "input_x2:0": x_dev_batch2,
                                   "dropout_keep_prob:0": 1.0, "weights:0": weights_d})
                results = np.concatenate([results, logits], axis=0)
                # if i == 0:
...
if __name__ == '__main__':
    _, _, _, _, _, _, word_embedding, max_len, vocab_size, x_test1, x_test2 = process()
    result = predict(x_test1, x_test2)
    timestamp = str(int(time.time()))
    file_name = timestamp + "result_cnn1d.txt"
