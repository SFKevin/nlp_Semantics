...
lgb_model = lgb.train(params=params, train_set=train_input, valid_sets=val_input, early_stopping_rounds=100)

print("predict")
pred = lgb_model.predict(data=test.values, num_iteration=lgb_model.best_iteration)
train_pred = lgb_model.predict(data=train.values, num_iteration=lgb_model.best_iteration)

np.savetxt("result_lgb20180729.txt", pred)
