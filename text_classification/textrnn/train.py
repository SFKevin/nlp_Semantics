...
def train(x_train, y_train, vocab_size, x_dev, y_dev, word_embeddings):
    """
    Training
    :param x_train:
    :param y_train:
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            textRnn = TextRNN(FLAGS.num_classes, 0.01, FLAGS.batch_size, FLAGS.decay_steps, FLAGS.decay_rate, 59,
            ...
            t_assign_embedding = tf.assign(textRnn.Embedding, embedding)
            ...
            def train_step(x_batch, y_batch):
                feed_dict = {textRnn.input_x: x_batch, textRnn.input_y: y_batch,
                             textRnn.dropout_keep_prob: FLAGS.dropout_keep_prob}
                _, step, loss, accuracy = sess.run(
                    [textRnn.train_op, textRnn.global_step, textRnn.loss_val, textRnn.accuracy], feed_dict)
                time_str = datetime.datetime.now().isoformat()
                print("{}: step {}, loss {:g}. acc {:g}".format(time_str, step, loss, accuracy))

            def dev_step(x_batch, y_batch, writer=None):
                feed_dict = {textRnn.input_x: x_batch, textRnn.input_y: y_batch, textRnn.dropout_keep_prob: 1.0}
                step, loss, accuracy = sess.run([textRnn.global_step, textRnn.loss_val, textRnn.accuracy],
                                                           feed_dict)
                time_str = datetime.datetime.now().isoformat()
                print("{}:step {}, loss: {:g}, acc: {:g}".format(time_str, step, loss, accuracy))
            ...
            for batch in batches:
                ...
                current_step = tf.train.global_step(sess, textRnn.global_step)
